...
int CCommonConvert::WStringToChar(const std::wstring str,char c_str[],int length)
{
	///参数传NULL，返回长度
	...
	if(c_str!=NULL)
	{
		...
		WideCharToMultiByte(CP_ACP,0,str.c_str(),str.length(),tmpch,len,NULL,NULL);
		strncpy_s(c_str,length,tmpch,len+1);
		c_str[len]='\0';
		delete [] tmpch;
	}
	return len;
...
int CCommonConvert::CharToWString(std::wstring& str,const char c_str[])
{
	
	int len = MultiByteToWideChar(CP_ACP,0,c_str,-1,const_cast<LPWSTR>(str.c_str()),0);
	if(c_str!=NULL)
	{
		wchar_t* w_str = NULL;
...
void CCommonConvert::intToWString(int i,std::wstring& str)
{
	char c_str[255]={0};
	itoa(i,c_str,10);
	wchar_t w_str[255]={0};
...
void CCommonConvert::WStringToint(int& i,const std::wstring str)
{
	char c_str[255]={0};
	WStringToChar(str,c_str,255);
	i=atoi(c_str);
}
/*
获取本机的IP地址
...
int CCommonConvert::Char2WChar(const char* source,size_t len,wchar_t* dest)
{
	int length = MultiByteToWideChar(CP_ACP,0,source,-1,dest,0);
	if(dest != NULL)
	{
		wchar_t* w_str = NULL;
		w_str = new wchar_t[length+1];
		ZeroMemory(w_str,length+1);
		MultiByteToWideChar(CP_ACP,0,source,-1,w_str,length);
		_tcsncpy_s(dest,len,w_str,length+1);
		delete [] w_str;
	}
	return length;
}
...
string CCommonConvert::WStringToString(const std::wstring str)
{
	int len = WStringToChar(str,NULL,0);
	char* szBuf = new char[len+1];
	memset(szBuf,0,len+1);
	WStringToChar(str,szBuf,len+1);
	std::string strResult(szBuf);
	delete [] szBuf;
	return strResult;
}

wstring CCommonConvert::StringToWString(const std::string str)
{
	std::wstring wStrResult;
	CharToWString(wStrResult,str.c_str());
	return wStrResult;
}