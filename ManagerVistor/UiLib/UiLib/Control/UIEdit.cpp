...
namespace UiLib
{
	...
	LRESULT CEditWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		//m_pOwner->m_pWindow = NULL;  //redrain修复bug
	...
	bool CEditUI::MatchRegular(bool isShowMsg/* = true*/)
	{
		...
		try
		{
			...
			boost::wregex regExpress(mSrcRegularCheck);
			...
			if(!boost::regex_match(mSrcText,regExpress))
				goto MatchFailed;
			else
				return true;
#else
			...
			boost::regex regExpress(mSrcRegularCheck);
			...
			if(!boost::regex_match(mSrcText,regExpress))
//			if(!boost::regmatch_t(mSrcText,regExpress))
				goto MatchFailed;
			else
				return true;
#endif
