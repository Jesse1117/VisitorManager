...
, m_strHtmlQueueInfoTableHead(_T(""))
...
{
	m_mapStatusInt_Str[qsDoing] = _T("呼叫/就诊");
	m_mapStatusInt_Str[qsPrepare] = _T("准备");
	m_mapStatusInt_Str[qsInLine] = _T("排队");
	...
	m_DbParam.SetDataDays(CommonStrMethod::GetPrivateProfileIntMy(
	...
	SetDataDays(m_DbParam.GetDataDays());
}
...
BOOL CNurseController::OnStartControl()
{
	...
	CommonStrMethod::CreatePathMy(m_strHtmlPath);
...
VerifyResultEnum CNurseController::VerifyDoctor(CString strDoctorID, 
												CString strPassword, 
...
void CNurseController::OnCall(LPDOCTORCMD pDoctorCmd, LPSOCKADDR_IN psockddr)
{
	...
	if(!sViewInfo.strSerialID.IsEmpty())
	{
		((CNurseStationApp*)AfxGetApp())->m_pNurseWnd->GetPatientView()->
			ReviseListCtrPatStatus(sViewInfo.strSerialID, qsDone, sViewInfo.strDoctorName);
	}
	...
	if(strQueSerialID.IsEmpty())
	{
		...
		CString strSql;
		strSql.Format(_T("select top 1 log_id,queue_id_call,patient_name,patient_gender,patient_month patient_age,status,doctor_id from Queue where doctor_id='%s' and status=%d and %s;")
		...
		try
		{
			...
			{
				rset.Close();
				if(docInfo.iExpertFlag == 1)
				{
					strSql.Format(_T("Select top 1 Q.log_id,Q.queue_id_call,Q.patient_name,Q.patient_gender,Q.patient_month patient_age,Q.status,Q.doctor_id from Queue Q, (select doctor_id, office_id from Doctor where doctor_id='%s') D where Q.office_id=D.office_id and Q.doctor_id=D.doctor_id and status=%d and %s order by Q.priority desc, Q.regtime;")
						,strDoctorID , qsInLine, m_strSqlDate);
				}
				else
				{
	...
	{
		SqlUpdateToCall(strDoctorID, strQueSerialID);
	}
	//
	//if(!bOurCaller)
	//{
	...
	//刷新CPaintView中的患者状态
	((CNurseStationApp*)AfxGetApp())->m_pNurseWnd->GetPatientView()->
		ReviseListCtrPatStatus(sViewInfo.strSerialID, qsDoing, sViewInfo.strDoctorName);
	...
	//((CNurseStationApp*)AfxGetApp())->m_pNurseWnd->GetPatientView()->ReviseListCtrPatStatus(
	//	cpiCall.strSerialID,qsDoing,cpiCall.sDoctorBasicInfo.strDoctorName);
...
void CNurseController::OnRecall(LPDOCTORCMD pDoctorCmd, LPSOCKADDR_IN psockddr)
{
	...
	strSqlSel.Format(_T("Select log_id from Queue where doctor_id='%s' and status=%d and %s;")
...
void CNurseController::OnDiscard(LPDOCTORCMD pDoctorCmd, LPSOCKADDR_IN psockddr)
{
	...
	CString strSql;
	strSql.Format(_T("Select log_id,queue_id_call,patient_name,patient_gender,patient_month patient_age,status,doctor_id from Queue where doctor_id='%s' and status=%d and %s;")
		,strDoctorID , qsDoing, m_strSqlDate);
	CString strSerialID;
	try
	{
	...
	if(!strSerialID.IsEmpty())
	{
		...
		//刷新CPaintView中的患者状态
		((CNurseStationApp*)AfxGetApp())->m_pNurseWnd->GetPatientView()->
			ReviseListCtrPatStatus(strSerialID, qsDiscard, m_mapDocIdDocInfo[strDoctorID].strDoctorName);
...
void CNurseController::OnWait(LPDOCTORCMD pDoctorCmd, LPSOCKADDR_IN psockddr)
{
	...
	if(strSerialID.IsEmpty())
	{
		...
		try
		{
			...
			if(rset.IsBOF())
			{
				...
				if(docInfo.iExpertFlag == 1)
				{
					strSql.Format(_T("Select Q.log_id,Q.queue_id_call,Q.patient_name,Q.patient_gender,Q.patient_month patient_age,Q.status,Q.doctor_id from Queue Q, (select doctor_id, office_id from Doctor where doctor_id='%s') D where Q.office_id=D.office_id and Q.doctor_id=D.doctor_id and status=%d and %s order by Q.priority desc, Q.regtime;")
						,strDoctorID , qsInLine, m_strSqlDate);
				}
				else
				{
	...
	{
		SqlUpdateToWait(strDoctorID, strSerialID);
	}
	...
	if(!strSerialID.IsEmpty())
	{
		...
		//刷新CPaintView中的患者状态
		((CNurseStationApp*)AfxGetApp())->m_pNurseWnd->GetPatientView()->
			ReviseListCtrPatStatus(strSerialID, qsPrepare, m_mapDocIdDocInfo[strDoctorID].strDoctorName);
	}
	else
	{
		bSuc = FALSE;
...
void CNurseController::AddToSoundAndDisplay(DOCTORBASICINFO& rDoctorBasicInfo, CString& strQueSerialID, BOOL bCall)
{
	...
	//vdstr.bScreen = TRUE;
	vdstr.strStbId = m_mapOffice[rDoctorBasicInfo.strOfficeID].GetStbId();
	PlayVoice(vdstr);
}
...
{
	CString strSql(_T(""));
	strSql.Format(_T("select Q.queue_id_call QueueID,Q.patient_name PatientName,O.call_name OfficeName,C.call_name ConsultName,D.name DoctorName,Q.status Status from Queue Q left join Office O on O.office_id=Q.office_id left join Doctor D on D.doctor_id=Q.doctor_id left join Consult C on C.consult_id=D.consult_id where %s and O.office_id='%s' and Status in (%d,%d,%d) order by Status,priority desc,regtime;"), 
	...
	try
	{
		...
		if(!rset.IsBOF())
		{
			CString strQueueID, strPatientName, strOfficeName, strConsultName, strDoctorName;
			int iStatus;
			for(; !rset.IsEOF(); rset.MoveNext())
			{
				rset.GetFieldValue(_T("QueueID"), strQueueID);
				rset.GetFieldValue(_T("PatientName"), strPatientName);
				rset.GetFieldValue(_T("OfficeName"), strOfficeName);
				rset.GetFieldValue(_T("ConsultName"), strConsultName);
				rset.GetFieldValue(_T("DoctorName"), strDoctorName);
				rset.GetFieldValue(_T("Status"), iStatus);
				//LPSTBPATIENT pPatient = new STBPATIENT;
				StbPatient patient;
				patient.strQueueId = strQueueID;
				patient.strPatientName = strPatientName;
				patient.strOfficeName = strOfficeName;
				patient.strConsultName = strConsultName;
				patient.strDoctorName = strDoctorName;
...
{
	CString strSql;
	strSql.Format(_T("select Q.log_id SerialID, Q.patient_name PatientName, Q.office_id, D.name DoctorName from Queue Q left join Doctor D on Q.doctor_id=D.doctor_id where Q.status=%d and Q.doctor_id='%s' and %s;"),
	...
	try
	{
		...
		rset.GetFieldValue(0, pViewUpdateInfo->strSerialID);
		rset.GetFieldValue(1, pViewUpdateInfo->strPatientName);
		rset.GetFieldValue(2, pViewUpdateInfo->strOfficeID);
		rset.GetFieldValue(3, pViewUpdateInfo->strDoctorName);
...
{
	CString strSql;
	strSql.Format(_T("select queue_id_call, patient_name, patient_gender, patient_age, status from Queue where log_id='%s';")
		, strSerialID);
	try
	{
		CADODatabase db;
		...
		if(rset.Open(strSql, CADORecordset::openQuery))
		{
			...
			rset.GetFieldValue(_T("patient_age"), pCallPatientInfo->iAge);
	...
	try
	{
		...
		if(rset.Open(strSql, CADORecordset::openQuery))
		{
			if(!rset.IsBOF())
			{
				...
				rset.GetFieldValue(_T("expert_flag"), rDoctorBasicInfo.iExpertFlag);
...
BOOL CNurseController::SendCmdResult(LPBASEINFO lpBaseInfo, const LPSOCKADDR_IN psockddr, BOOL bSucceed)
{
	...
	if(lpBaseInfo->header.eTypeID == ntiDoctorCmd)
	{
		...
		if(eCmd == dcCmdCall || eCmd == dcCmdWait || eCmd == dcCmdDiscard)
		{
			SqlQueryPatients(strDoctorID, &cmdres);
		}
		//else
		//{
		//	strSql.Format(_T("Select count(*) PatientNum from Queue where doctor_id='%s' and (status=%d or status=%d) and convert(varchar(10),getdate(),120)=convert(varchar(10),regtime,120);"), strDoctorID , qsInLine, qsPrepare);
		//	if(rset.Open(m_db.m_pConnection, strSql)
		//	{
		//		if(!rset.IsBOF())
		//		{
...
BOOL CNurseController::SqlQueryPatients(CString& strDoctorId, LPCMDRESULT pCmdResult)
{
	...
	try
	{
		...
		//	if(!rset.IsBOF())
		//	{
		//		CString strSerialID, strQueueID, strPatientName,strPatientGender,strPatientAge;
		//		rset.GetFieldValue(_T("log_id"), strSerialID);
		//		rset.GetFieldValue(_T("queue_id_call"), strQueueID);
		//		rset.GetFieldValue(_T("patient_name"), strPatientName);
		//		wcscpy_s(pCmdResult->patientsArray[nIndex].wszSerialID, MAX_ID_LENGTH, strSerialID.GetBuffer());
		//		wcscpy_s(pCmdResult->patientsArray[nIndex].wszQueueID, MAX_ID_LENGTH, strQueueID.GetBuffer());
		//		wcscpy_s(pCmdResult->patientsArray[nIndex].wszPatientName, MAX_NAME_LENGTH, strPatientName.GetBuffer());
		//		int iGender;
		//		rset.GetFieldValue(_T("patient_gender"), iGender);
		//		strPatientGender = GetGenderStr(iGender);
		//		wcscpy_s(pCmdResult->patientsArray[nIndex].wszPatientGender, 8, strPatientGender.GetBuffer());
		//		int iAge;
		//		rset.GetFieldValue(_T("patient_age"), iAge);
		//		strPatientAge = GetAgeStr(iAge);
		//		wcscpy_s(pCmdResult->patientsArray[nIndex].wszPatientAge, 16, strPatientAge.GetBuffer());

		//		pCmdResult->patientsArray[nIndex].eStatus = qsDoing;
		//		nIndex++;
		//	}
		//	rset.Close();
		//}
		...
		////strSql.Format(_T("Select log_id,his_queue_id queue_id_call,patient_name,patient_gender,patient_month patient_age,status,doctor_id from Queue where doctor_id='%s' and status=%d;"),strDoctorID , qsPrepare);
		//strSql.Format(_T("Select log_id,queue_id_call,patient_name,patient_gender,patient_month patient_age,status,doctor_id from Queue where doctor_id='%s' and status=%d and %s order by priority desc, regtime;"),
		//	strDoctorId , qsPrepare, m_strSqlDate);
		//if(rset.Open(strSql))
		//{
		//	if(!rset.IsBOF())
		//	{
		//		uCount += rset.GetRecordCount();
		//		for(; !rset.IsEOF() && nIndex < MAX_PATIENTS_SHOW; rset.MoveNext())
		//		{
		//			CString strSerialID, strQueueID, strPatientName,strPatientGender,strPatientAge;
		//			rset.GetFieldValue(_T("log_id"), strSerialID);
		//			rset.GetFieldValue(_T("queue_id_call"), strQueueID);
		...
		//			strPatientAge = GetAgeStr(iAge);
		//			wcscpy_s(pCmdResult->patientsArray[nIndex].wszPatientAge, 16, strPatientAge.GetBuffer());
		//			pCmdResult->patientsArray[nIndex].eStatus = qsPrepare;
		//			nIndex++;
		//			//uCount++;
		//		}
		//	}
		...
		////strSql.Format(_T("Select Q.log_id,Q.his_queue_id queue_id_call,Q.patient_name,Q.patient_gender,Q.patient_month patient_age,Q.status,Q.doctor_id from Queue Q, (select doctor_id, office_id from Doctor where doctor_id='%s') D where Q.office_id=D.office_id and (Q.doctor_id is null or Q.doctor_id='' or Q.doctor_id=D.doctor_id) and status=%d and convert(varchar(10),getdate(),120)=convert(varchar(10),Q.regtime,120) order by Q.priority desc, Q.regtime;"),strDoctorID , qsInLine);
		//strSql.Format(_T("Select Q.log_id,queue_id_call,Q.patient_name,Q.patient_gender,Q.patient_month patient_age,Q.status,Q.doctor_id from Queue Q, (select doctor_id, office_id from Doctor where doctor_id='%s') D where Q.office_id=D.office_id and (Q.doctor_id is null or Q.doctor_id='' or Q.doctor_id=D.doctor_id) and status=%d and %s order by Q.priority desc, Q.regtime;"), 
		//	strDoctorId , qsInLine, m_strSqlDate);
		//if(rset.Open(strSql))
		//{
		//	if(!rset.IsBOF())
		...
		//			strPatientAge = GetAgeStr(iAge);
		//			wcscpy_s(pCmdResult->patientsArray[nIndex].wszPatientAge, 16, strPatientAge.GetBuffer());
		//			pCmdResult->patientsArray[nIndex].eStatus = qsInLine;
		//			nIndex++;
		//		}
		//	}
		//	rset.Close();
...
{
	CString strPath = m_strHtmlPath + _T("/") + strStbId + _T("/") + _T("webinfo2.html");
	return CommonStrMethod::PathFileExist(strPath);
}
...
BOOL CNurseController::StbUpdateTitleHtml(CString strTitle, CString strStbId)
{
	...
	CString strExePath = CommonStrMethod::GetModuleDir() + _T("sys\\stb.ini");
...
BOOL CNurseController::StbUpdateQueueInfoHtmlTypeRow(CString strStbId)
{
	...
	if(bMultiOffice)
	{
		//strTableHead = _T("<tr><th></th><th>科室</th><th>排队号</th><th>患者</th><th>医生</th><th>诊室</th><th>状态</th></tr>");
		strTableHead += _T("<th>科室</th><th>排队号</th><th>患者</th><th>诊室</th><th>状态</th></tr>");
	}
	else
	{
		strTableHead += _T("<th>排队号</th><th>患者</th><th>诊室</th><th>状态</th></tr>");
	}
	...
	for(int i = 0; i < m_iHtmlPageItemCount && m_mapStb[strStbId].nIndex < rowList.GetCount(); i++, m_mapStb[strStbId].nIndex++)
	{
		...
		{
			strRowColor = _T("<font color=\"Red\">");
			strStatus = _T("呼叫");
		}
		else if(rowList[m_mapStb[strStbId].nIndex].eStatus == qsPrepare)
		{
			strRowColor = _T("<font color=\"Orange\">");
			strStatus = _T("准备");
		}
		else
		{
			strRowColor = _T("<font color=\"black\">");
			strStatus = _T("排队");
		}
		if(m_bShowLineNum)
		{
			strRow += _T("<td>");
			strRow += CommonStrMethod::Int2Str(m_mapStb[strStbId].nIndex + 1);
			strRow += _T("</td>");
		}
		...
		strRow += _T("</font></td>");
		//strRow += _T("<td>") + strRowColor;
		//strRow += rowList[nIndex].strDoctorName;
		//strRow += _T("</font></td>");
		strRow += _T("<td>") + strRowColor;
...
BOOL CNurseController::StbUpdateQueueInfoHtmlTypeCol(CString strStbId)
{
	...
	for(int i = 0; i < m_mapStb[strStbId].arrOffice.GetCount(); i++)
	{
		...
		strTableHead = strTableHead + _T("<th>") + m_mapOffice[m_mapStb[strStbId].arrOffice[i]].GetOfficeName() + _T("(<font color=\"Green\">")
			+ CommonStrMethod::Int2Str(m_mapOffice[m_mapStb[strStbId].arrOffice[i]].GetQueueNum())  + _T("</font>人)</th>");
	}
	strTableHead += _T("</tr>");
	CString strHtml(m_strHtmlQueueInfoBegin + strTableHead);
...
BOOL CNurseController::StbUpdateQueueInfoHtmlTypeSwitchCol(CString strStbId)
{
	...
	{
		strTableHead = strTableHead + _T("<th>") + m_mapOffice[m_mapStb[strStbId].arrOffice[nColTemp]].GetOfficeName() + _T("(<font color=\"Green\">")
			+ CommonStrMethod::Int2Str(m_mapOffice[m_mapStb[strStbId].arrOffice[nColTemp]].GetQueueNum())  + _T("</font>人)</th>");
	}
	strTableHead += _T("</tr>");
	...
	for(int i = 0; i < m_iHtmlPageItemCount; i++)
	{
		...
		{
			strRow += _T("<td>");
			strRow += CommonStrMethod::Int2Str(i + 1);
			strRow += _T("</td>");
		}
...
{
	//return ::CopyFile(m_strHtmlPath + _T("/webinfo.html"), 
	//	m_strHtmlPath + _T("/") + strStbId + _T("/webinfo.html"), FALSE);
...
BOOL CNurseController::WriteToStbHtmlFile(CString& strHtmlStr, CString strStbId, CString strHtmlFileName)
{
	...
	if(strStbId != _T("0"))
	{
		CString strPath = m_strHtmlPath + _T("/") + strStbId;
		if(CommonStrMethod::CreatePathMy(strPath))
		{
	...
	else
	{
		...
		{
			CString strPath = m_strHtmlPath + _T("/") + m_arrstrStbIdList[i];
			if(CommonStrMethod::CreatePathMy(strPath))
			{
				strPath = strPath + _T("/") + strHtmlFileName;
				bRet = WriteToFile(strHtmlStr, strPath);
								...
								CALLPATIENTINFO& cpiCall)
{
	...
	strDest = StringReplace(strSrc, STRING_QUEUEID_CALL, cpiCall.strQueueID);
	strDest = StringReplace(strDest, STRING_PATIENTNAME_CALL, cpiCall.strPatientName);
	strDest = StringReplace(strDest, STRING_DOCTORNAME, cpiCall.sDoctorBasicInfo.strDoctorName);
	strDest = StringReplace(strDest, STRING_OFFICENAME, cpiCall.sDoctorBasicInfo.strOfficeName);
	strDest = StringReplace(strDest, STRING_CONSULTNAME, cpiCall.sDoctorBasicInfo.strConsultName);
	strDest = StringReplace(strDest, STRING_PATIENT_GENDER, GetGenderStr(cpiCall.eGender));
	strDest = StringReplace(strDest, STRING_PATIENT_AGE, GetAgeStr(cpiCall.iAge));
	return strDest;
}


CString CNurseController::StringReplace(CString strSrc, CString strFrom, CString strTo)
{
	...
	BOOL bFound = FALSE;
	int nStart = -1;
	for(; (nStart = strSrc.Find(strFrom.GetAt(0))) != -1;)
	{
		int i = 0, j = nStart;
		for(; i < nLength-1;)
		{
			if(strSrc.GetAt(++j) != strFrom.GetAt(++i))
			{
	...
	CString strDest = strSrcTemp.Left(nAbsStart);
	strDest += strTo;
	strDest += strSrcTemp.Mid(nAbsStart + nLength);
	//strDest += strSrc.Mid(nStart+1);

	return StringReplace(strDest, strFrom, strTo);
}

void CNurseController::LoadDisplayConf()
{
	CString strStbPath = CommonStrMethod::GetModuleDir() 
		+ _T("sys\\stb.ini");

	m_strHtmlTitleBegin = _T("<!DOCTYPE HTML><html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" /><style type=\"text/css\">body{text-align: left; color: #ffffcc; float:left; font:  bold 60px/60px Verdana, Geneva, sans-serif;}#center{ margin-left: auto; margin-right: auto; vertical-align: middle; }</style><title>医院标题</title></head><body><div style=\"textalign:left\"><img src=\"../img/hspt.jpg\" width=\"60\" height=\"60\" style=\"float:left\">");
	m_strHtmlTitleBegin = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("TITLEBEGIN"), m_strHtmlTitleBegin, strStbPath);
	m_strHtmlTitleEnd = _T("</div></body></html>");
	m_strHtmlTitleEnd = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("TITLEEND"), m_strHtmlTitleEnd, strStbPath);
	m_strHtmlNoticeBegin = _T("<!DOCTYPE HTML><html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" /><style type=\"text/css\">body{text-align: center; color: #ff0000; font:  bold 50px/50px Verdana, Geneva, sans-serif;}#center{ margin-left: auto; margin-right: auto; vertical-align: middle; }</style><title>通知</title></head><body><div><marquee scrollamount=\"7\">");
	m_strHtmlNoticeBegin = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("NOTICEBEGIN"), m_strHtmlNoticeBegin, strStbPath);
	m_strHtmlNoticeEnd = _T("</marquee></div></body></html>");
	m_strHtmlNoticeEnd = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("NOTICEEND"), m_strHtmlNoticeEnd, strStbPath);
	...
	m_strHtmlInstMsgBegin = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("INSTMSGBEGIN"), m_strHtmlInstMsgBegin, strStbPath);
	m_strHtmlInstMsgEnd = _T("</div></body></html>");
	m_strHtmlInstMsgEnd = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("INSTMSGEND"), m_strHtmlInstMsgEnd, strStbPath);
	m_strHtmlQueueInfoBegin = _T("<!DOCTYPE HTML><html><head><meta content=\"text/html; charset=utf-8\" http-equiv=\"Content-Type\" /><style type=\"text/css\">body{text-align: center; color: #000000; font:bold 50px/50px Verdana, Geneva, sans-serif;}#center{ margin-left: auto; margin-right: auto; vertical-align: middle; }#customers{font-family:\"Trebuchet MS\", Arial, Helvetica, sans-serif;width:100%;border-collapse:collapse;}#customers td, #customers th {font-size:1em;border:1px solid #98bf21;padding:3px 7px 2px 7px;}#customers th {font-size:1.1em;padding-top:5px;padding-bottom:4px;background-color:#A7C942;color:#ffffff;}#customers tr.ord td {color:#000000;background-color:#FAF0E6;}#customers tr.alt td {color:#000000;background-color:#EAF2D3;}</style><title>排队信息</title></head><body><div style=\"width: 1280px; margin-left: 0%; margin-right: auto; margin-top: 0px;\"><div><table id=\"customers\"; width=\"2000px\"; border=\"1\" cellpadding=\"0\" cellspacing=\"0\">");
	m_strHtmlQueueInfoBegin = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("QUEUEINFOBEGIN"), m_strHtmlQueueInfoBegin, strStbPath);
	m_strHtmlQueueInfoTableHead = _T("<tr><th>排队号</th><th>患者</th><th>科室</th><th>诊室</th><th>状态</th></tr>");
	m_strHtmlQueueInfoTableHead = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("QUEUEINFOTABLEHEAD"), m_strHtmlQueueInfoTableHead, strStbPath);
	m_strHtmlQueueInfoEnd = _T("</table></div></div></body></html>");
	m_strHtmlQueueInfoEnd = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("QUEUEINFOEND"), m_strHtmlQueueInfoEnd, strStbPath);
	m_strHtmlTitle = CommonStrMethod::GetPrivateProfileStringMy(
		_T("sys"),_T("TITLE"), _T("XXX医院XX科"), strStbPath);

	m_iHtmlPageItemCount = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"),_T("PAGE_ITEM_COUNT"), 8, _T("\\SYS\\sysconfig.ini"));
	m_iHtmlPageSwitchTime = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"),_T("PAGE_SWITCH_TIME"), 8, _T("\\SYS\\sysconfig.ini"));
	m_iHtmlShowCallTime = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"),_T("SHOW_CALL_TIME"), 8, _T("\\SYS\\sysconfig.ini"));
	m_nShowType = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"), _T("SHOW_TYPE"), 0, _T("\\SYS\\sysconfig.ini"));
	m_bShowQueueNum = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"), _T("SHOW_QUEUE_NUM"), 1, _T("\\SYS\\sysconfig.ini"));
	m_bSwitchPage = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"), _T("SWITCH_PAGE"), 1, _T("\\SYS\\sysconfig.ini"));
	m_nPageColCount = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"), _T("PAGE_COL_COUNT"), 3, _T("\\SYS\\sysconfig.ini"));
	m_bShowLineNum = CommonStrMethod::GetPrivateProfileIntMy(
...
BOOL CNurseController::InitDisplay()
{
	...
	CString strSql = _T("select stb_id, stb_init_text from SetTopBox where 1=1;");
	...
	{
		CString strPath = m_strHtmlPath + _T("/") + m_arrstrStbIdList[i];
		CommonStrMethod::CreatePathMy(strPath);
	}

	CommonStrMethod::CreateFileMy(m_strHtmlPath + _T("/img/hspt.jpg"));
	...
	for(;pos;)
	{
		...
		if(!office.GetStbId().IsEmpty())
		{
			StbData stbData;
			if(m_mapStb.Lookup(office.GetStbId(), stbData))
			{		
				m_mapStb[office.GetStbId()].arrOffice.Add(office.GetOfficeId());
			}
			else
			{
				StbData stbDt;
				stbDt.arrOffice.Add(office.GetOfficeId());
				m_mapStb[office.GetStbId()] = stbDt;
			}
		}
	}
...
UINT CNurseController::DisplayThreadProc(LPVOID pParam)
{
	...
	while(pThis->m_bDisplayThread)
	{
		...
		for(;pPair;)
		{
			...
			pPair = pThis->m_mapStb.PGetNextAssoc(pPair);
		}
...
CString CNurseController::GetAgeStr(int iAge)
{
	...
	if(iYear > 0)
	{
		strPatientAge.Format(_T("%d岁"), iYear);
	}
	if(iMonth > 0)
	{
		strPatientAge.AppendFormat(_T("%d月"), iMonth);
	}
...
BOOL CNurseController::InitNetServer()
{
	...
	if(!hWndNetServer)
	{
		...
		//ShExecInfo.lpDirectory = CommonStrMethod::GetModuleDir();
		//ShExecInfo.nShow = SW_SHOW;
		//ShExecInfo.hInstApp = NULL;
		//ShellExecuteEx(&ShExecInfo);
		////::WaitForSingleObject(ShExecInfo.hProcess,INFINITE);

		//ShellExecute(NULL,_T("open"),CommonStrMethod::GetModuleDir() + _T("CassiniDev.exe"),NULL,NULL, SW_SHOW );
		...
		CString strCmdLine;
		//strCmdLine.Format(_T("\"%sCassiniDev.exe\""), CommonStrMethod::GetModuleDir());
		strCmdLine = _T("CassiniDev.exe");

		//if(!::CreateProcess(NULL, strCmdLine.GetBuffer(), NULL, NULL, TRUE, NULL, NULL, CommonStrMethod::GetModuleDir(), &si, &m_piNetServer))
		if(!::CreateProcess(NULL, strCmdLine.GetBuffer(), NULL, NULL, FALSE, NULL, NULL, CommonStrMethod::GetModuleDir(), &si, &m_piNetServer))
		{
			bRet = FALSE;
			AfxMessageBox(_T("多媒体信息发布服务端无法启动，请手动启动!"));
		}
		//else
		//{
...
void CNurseController::AutoClearData()
{
	CString strDel;
	strDel.Format(_T("delete from Queue where regtime<cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime);"), m_nDataDays + 1);
	try
	{
		CADODatabase db;
		db.Open(m_strSqlConn);
	...
	{
		WriteLog::WriteDbErrLog(_T("CNurseController::AutoClearData"));
	}
}
