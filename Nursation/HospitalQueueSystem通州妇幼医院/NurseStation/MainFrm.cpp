...
#include "NurseDef.h"
//#include "UiFlushTimeDialog.h"
#include "PrintSetDlg.h"
//#include "TakingNumConfDlg.h"
...
//extern CDoctorCallerDlg* g_pDoctorCallerDlg;

// 用于应用程序“关于”菜单项的 CAboutDlg2 对话框
...
CAboutDlg2::CAboutDlg2() : CDialog(CAboutDlg2::IDD)
{
}
...
//	DockControlBar(&m_wndToolBar);
	...
	CString time = CommonStrMethod::GetPrivateProfileStringMy(
	...
	//////////////////////////////////////////////////////////////////////////
// 	CRect rect(0,0,256,64);
 	m_pVisonShowLED = new CVisonShowLED(this->m_hWnd);//设置北京通屏接收消息的窗体句柄
//	m_pVisonShowLED->SendDataToScreen(_T("明月几时有？把酒问青天。不知天上宫阙，今夕是何年。我欲乘风归去，又恐琼楼玉宇，高处不胜寒。起舞弄清影，何似在人间？转朱阁，低绮户，照无眠。不应有恨，何事长向别时圆？人有悲欢离合，月有阴晴圆缺，此事古难全。但愿人长久，千里共婵娟。"),
//		_T("192.168.0.99"),rect,10*1000);
	return 0;
}
...
CMainFrame::CMainFrame() : m_pVisonShowLED(NULL)
{
	m_nurse=NULL;
	m_pDocview=NULL;
	m_pOfficeview=NULL;
...
//	m_hComWndScreen=INVALID_HANDLE_VALUE;
	...
	//SetDataDays(CommonStrMethod::GetPrivateProfileIntMy(
	//	_T("sys"), _T("DATA_DAYS"), 0, _T("\\SYS\\sysconfig.ini")));

	//m_strSqlDate.Format(_T("regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime)"), m_nDataDays);
	m_bReadInPatient = 1 == CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"), _T("READ_INPATIENT"), 1, _T("\\SYS\\sysconfig.ini"));
...
CMainFrame::~CMainFrame()
{
	...
	*/
	...
	if(m_pVisonShowLED)
	{
		delete m_pVisonShowLED;
		m_pVisonShowLED = NULL;
	}
}
...
// 	m_hDoCallerMsgThread=CreateThread(NULL,0,DoCallerMsgThread,this,NULL,0);
	...
	//g_nDataDays = CommonStrMethod::GetPrivateProfileIntMy(_T("sys"), _T("DATA_DAYS"), 0, _T("\\SYS\\sysconfig.ini"));
...
BOOL CMainFrame::WriteDataToMyQueue()
{
	...
	try
	{
		...
		CString sql=_T("");
		db.Open();
		//sql.Format(_T("select * from (select distinct h.serial_id,h.patient_name,h.queue_num,h.id_depart,h.id_doctor,h.time,h.patient_gender,h.pay_flag,o.name officename,o.call_id from HisForBjlb_Patient h left join Office o on o.office_id=h.id_depart left join Doctor d on h.id_doctor=d.doctor_id)h where h.time>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) and h.serial_id not in (select log_id from Queue) and h.id_depart in(select o.office_id from Nurse_Office where Nurse_id='%s') order by h.time"),
		//	CommonStrMethod::GetPrivateProfileIntMy(_T("sys"), _T("DATA_DAYS"), 0, _T("\\SYS\\sysconfig.ini")), theApp.GetLoginID());
		sql.Format(_T("select h.serial_id,h.patient_name,h.queue_num,h.id_depart,h.id_doctor,h.time,h.patient_gender,h.pay_flag,h.id_pay,h.pay_desc,h.read_flag,h.in_flag,o.name officename,o.call_id from HisForBjlb_Patient h left join Office o on o.office_id=h.id_depart left join Doctor d on h.id_doctor=d.doctor_id where h.time>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) and (h.read_flag is null or h.read_flag=0) and h.serial_id not in (select log_id from Queue) and h.id_depart in(select office_id from Nurse_Office where Nurse_id='%s') order by h.time"),
		...
		while(!pre->IsEOF())
		{
			try
			{
				...
				//			pre->GetFieldValue(_T("month"),patientMonth);
				pre->GetFieldValue(_T("patient_gender"),gender);

				pre->GetFieldValue(_T("id_pay"), strIdPay);
				pre->GetFieldValue(_T("pay_desc"), strPayDesc);
					...
					{	
						...
						if(CurrentTime<=AmTime)
						{
							...
							if(judge)
							{
								...
								if(docID!=_T(""))
								{
									insertSql.Format(_T("insert into Queue(log_id,queue_id,patient_name,office_id,doctor_id,regtime,status,priority,queue_id_call,his_queue_id,patient_month,patient_gender,id_pay,pay_desc,in_flag) values('%s','%d','%s','%s','%s','%s','%d','%d','%s','%s','%d','%d','%s','%s',%d)"),
										log_ID,NumStart,deptName,Queue,docID,regTime,qsInLine,priZero,queue_id_his,queue_id_his,patientMonth,myGender,strIdPay,strPayDesc,nInFlag);
								}
								else
								{
									insertSql.Format(_T("insert into Queue(log_id,queue_id,patient_name,office_id,regtime,status,priority,queue_id_call,his_queue_id,patient_month,patient_gender,id_pay,pay_desc,in_flag) values('%s','%d','%s','%s','%s','%d','%d','%s','%s','%d','%d','%s','%s',%d)"),
										log_ID,NumStart,deptName,Queue,regTime,qsInLine,priZero,queue_id_his,queue_id_his,patientMonth,myGender,strIdPay,strPayDesc,nInFlag);
								}
...
int CMainFrame::WriteFileFromPat(CString strFilePath)
{
	...
	CString strLines(_T("序号,排队号码,患者姓名,就诊号,挂号时间,医生姓名,当前状态,等侯时间,优先级\r\n"));
	CString context=_T("");
...
/*
void CMainFrame::DoCallerMsg(char* buf,int size,CADODatabase &threadBase)
{
	int CallerAddress=buf[5];//呼叫器地址
	CString docid=_T("");
	CString docname=_T("");
	CString sql=_T("");
	sql.Format(_T("select d.doctor_id,d.name from Doctor d where d.caller_id='%d'"),
		CallerAddress);
	CADORecordset *pre=new CADORecordset(&threadBase);
	if(!pre->Open(sql,CADORecordset::openQuery))
	{
		return;
	}
	...
	{
		//呼叫next
		//SendMsgToWndScreen(docid,_T("呼叫"));
		
		
// 		if(!pre->IsBOF())
// 		{
// 			waitCount=pre->GetRecordCount();
// 		}
// 	
// 		SqlQueryInLine(*pre,docid.GetBuffer());
// 		if(!pre->IsBOF())
// 		{
// 			waitCount+=pre->GetRecordCount();
// 		}
...
// 		if(waitCount==0)
// 		{
// 			AlarmToSystem(CallerAddress);
// 		}
// 		else
// 		{	
// 			ShowLastPaintCount(CallerAddress,waitCount-1);
// 		}
		
		
		((CNurseStationApp*)AfxGetApp())->GetController()->SendCmd(docid, _T(""), dcCmdCall);
		//CString queNum = SqlQueryCallingQueueID(docid);
		//ShowCallerNum(CallerAddress,queNum);
	}
	...
	if(buf[size-3]==HARDWARE_CALLER_WAIT && size==10)
	{
		//等候
		//SendMsgToWndScreen(docid,_T("等候"));  
	...
	if(buf[size-3]==HARDWARE_CALLER_FUNCATION)
	{
		...
		{
			ShowCallerAddress(CallerAddress);
		}
		//5+功能-----过号
		if(size==11 && buf[size-4]==0x05)
		{
			//SendMsgToWndScreen(docid,_T("过号"));
			SqlQueryWait(*pre,docid.GetBuffer());
			if(!pre->IsBOF())
			{
				waitCount=pre->GetRecordCount();
			}
			...
			((CNurseStationApp*)AfxGetApp())->GetController()->SendCmd(docid, _T(""), dcCmdDiscard);
		}
		//4+功能-----退出登录
		if(size==11 && buf[size-4]==0x04)
		{
			//SendMsgToWndScreen(docid,_T("退出"));
		}
		//1+功能-----警报
		if(size==11 && buf[size-4]==0x01)
		{
			//SendMsgToWndScreen(docid,_T("警报"));
			AlarmToSystem(CallerAddress);
		}
		//柜员号+功能 登录
		if(size>11)
		...
		{
			SendMsgToWndScreen(docid,_T("暂停服务"));
		}
	}
	...
	{
		//重呼
		//SendMsgToWndScreen(docid,_T("重呼"));
...
void CMainFrame::SearchDoNum()
{
	try
	{
		...
		sql.Format(_T("select count(status)number from queue where office_id='%s' and status in('%d','%d','%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s")
			,m_pOfficeview->GetOfficeID(),qsDoing,qsPrepare,qsInLine, theApp.GetController()->GetDataDays()
			, m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
		if(pre->Open(sql,CADORecordset::openQuery))
		{
			if(!pre->IsBOF())
		...
		sql.Format(_T("select count(status)number from queue where office_id='%s' and status='%d' and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s")
		...
		sql.Format(_T("select count(status)number from queue where office_id='%s' and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s")
