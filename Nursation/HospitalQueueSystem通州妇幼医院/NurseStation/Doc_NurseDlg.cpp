...
//CDoc_NurseDlg* g_Doc_NurseDlg;

// 用于应用程序“关于”菜单项的 CAboutDlg3 对话框

class CAboutDlg3 : public CDialog
{
public:
	CAboutDlg3();
...
CAboutDlg3::CAboutDlg3() : CDialog(CAboutDlg3::IDD)
{
}

void CAboutDlg3::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CAboutDlg3, CDialog)
END_MESSAGE_MAP()
...
BOOL CDoc_NurseDlg::ConnectToBase()
{
	...
	try
	{
		//CString strSql(_T("Driver={"+m_baseDrive+"};Server="+m_baseIp+";Database=Nurse;Uid="+m_baseAccount+";Pwd="+m_basePassword+";"));
		//pdataBase->SetConnectionString(strSql);
...
//			OnBnClickedOk();
	...
	case vrFoundOffice:
		{
			m_bCanLogin = FALSE;
			MessageBox(_T("该帐号与某已登录帐号的可处理科室存在重复"),_T("登录错误"),MB_OK | MB_ICONERROR);
			PostMessage(WM_COMMAND,IDOK,NULL);
		}
		break;
	}
