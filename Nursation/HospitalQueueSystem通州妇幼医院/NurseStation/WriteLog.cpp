...
{
	static int nWriteLog = CommonStrMethod::GetPrivateProfileIntMy(
	...
	CommonStrMethod::WChar2Char(szBuf, strLog.GetLength()*2 + 1, strLog.GetBuffer());
	CTime time = CTime::GetCurrentTime();
	static CString strLogPath = CommonStrMethod::GetModuleDir() + _T("log");
	if(!CommonStrMethod::PathFileExist(strLogPath))
	{
		CommonStrMethod::CreatePathMy(strLogPath);
	}
