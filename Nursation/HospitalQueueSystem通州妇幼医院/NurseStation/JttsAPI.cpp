...
UINT CJttsAPI::PlayVoiceThread(LPVOID pParam)
{
	...
	//while(1)
	{
		if(pThis->m_lstPlayStrList.IsEmpty())
		{
			...
			//((CNurseStationApp*)AfxGetApp())->m_pNurseWnd->SendMsgToWndScreen(
			//	vdstr.strDoctorID, vdstr.strDisplayStr);
			//ws.DoWndScreenMsg(vdstr.iWndScreenId, vdstr.strDisplayStr);

			((CNurseStationApp*)AfxGetApp())->GetController()->StbUpdateCallInfo(vdstr.strDisplayStr, vdstr.strStbId);
			//((CNurseStationApp*)AfxGetApp())->GetController()->StbUpdateInstantMsg(
			//	vdstr.strVoiceStr, vdstr.strConsultStbId);

			int nLength = vdstr.strVoiceStr.GetLength() * 2 + 1;
			char* buf = (char*)malloc(nLength);
			memset(buf, 0, nLength);
			CommonStrMethod::WChar2Char(buf, nLength, vdstr.strVoiceStr.GetBuffer());
			for(int i = 0; i < pThis->m_iSoundReplayTimes; i++)
			{
				jTTS_Play(buf, PLAYMODE_SYNC);
			}
			free(buf);
	...
	::ExitThread(0);
	//AfxMessageBox(_T("语音线程安全退出"));
	//if(pThis->m_pevtKillThread)
	//{
	//	pThis->m_pevtKillThread->SetEvent();
	//}
	return 0;
}
/*
void CJttsAPI::ErrorMessage(ERRCODE err)
{
...
{
	m_iSoundReplayTimes = CommonStrMethod::GetPrivateProfileIntMy(
		_T("sys"),_T("SOUND_REPLAY_TIMES"), 1, _T("\\SYS\\sysconfig.ini"));
}
