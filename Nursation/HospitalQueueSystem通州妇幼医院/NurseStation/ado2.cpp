...
BOOL CADODatabase::Open(LPCTSTR lpstrConnection, LPCTSTR lpstrUserID, LPCTSTR lpstrPassword)
{
	...
	if(StrCmpW(lpstrConnection, _T("")) != 0)
...
BOOL CADORecordset::GetFieldInfo(FieldPtr pField, CADOFieldInfo* fldInfo)
{
	...
	_tcscpy_s(fldInfo->m_strName,_tcslen(pField->GetName())+1,(LPCTSTR)pField->GetName());
...
BOOL CADORecordset::SetFieldValueByColNum(int nIndex, CString strValue)
{
	_variant_t vtFld;
	_variant_t vtIndex;	
...
void CADORecordset::dump_com_error(_com_error &e)
{
	...
	_bstr_t bstrSource(e.Source());
	_bstr_t bstrDescription(e.Description());
	CString strSource=(LPCTSTR)bstrSource;
	CString strDescription=(LPCTSTR)bstrDescription;
