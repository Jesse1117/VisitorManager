...
BOOL CPropQueSettting::OnInitDialog()
{
	...
	m_list_consult.InsertColumn(0,_T("诊室编号"),LVCFMT_LEFT,15*rect.Width()/100,-1);
	...
	m_list_consult.InsertColumn(6,_T("通屏地址"),LVCFMT_LEFT,10*rect.Width()/100,-1);
	m_list_consult.InsertColumn(7,_T("机顶盒"),LVCFMT_LEFT,10*rect.Width()/100,-1);
			...
			while(pos)
			{
				...
				while(posconsult)
				{
					...
					if(pre->Open(sql,CADORecordset::openQuery))
					{
						if(pre->IsBOF())
						{
							sql.Format(_T("insert into consult(consult_id,name,call_name,wnd_screen_id,compreh_screen_id,office_id,through_screen_id,through_screen_ip,stb_id) values('%s','%s','%s','%d','%d','%s','%d','%s','%s')"),
								...
								consult.GetTroughAddress(),consult.GetThroughIP(),consult.GetstdId());
							try
							{
								m_pbase->Execute(sql);
							}
						...
						{
							sql.Format(_T("update consult set name='%s',call_name='%s',wnd_screen_id='%d',compreh_screen_id='%d',office_id='%s',through_screen_id='%d',through_screen_ip='%s',stb_id='%s' where consult_id='%s'"),
								...
								consult.GetThroughIP(),consult.GetstdId(),consult.GetConsultID());
							try
							{
								m_pbase->Execute(sql);
							}
...
//				m_list_consult.SetItemText(item,7,consult.GetstdId());
				...
				for(int i=0;i<count;i++)
				{
					...
					{
						m_list_consult.SetItemText(item,7,m_mapto_consultStdInfo[i].
							m_stdName);
						break;
					}
				}
...
//	m_poolManage.CStringToint(m_selectConsultID,consultID);
	...
	for(int i=0;i<count;i++)
	{
		if(consult.GetstdId() == m_mapto_consultStdInfo[i].m_stdId)
		{
...
void CPropQueSettting::OnBnClickedBnAddconsult()
{
	...
	//m_consultID++;
	CConsult addConsult;
	//addConsult.SetConsultID(CommonStrMethod::Int2Str(m_consultID));
	...
	while(pos)
	{
		...
		if(myNurseOfficeQue.GetQueID()==GetSelectOfficeID())
		{
			...
			m_list_consult.SetItemText(m_consultItem-1,6,through_screen_id);
...
void CPropQueSettting::FlushMapToStbInfo()
{
	...
	///////////////////////////
	CString sql=_T("select * from settopbox");
