...
BOOL COfficeQueView::SearchData(CString officeid)
{
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDone,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDiscard,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_UNCHECKED
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_UNCHECKED
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d'or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsSuspend,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//3
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_UNCHECKED
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_UNCHECKED
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_UNCHECKED
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d'or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));	
	}//4
		...
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWWAITTING]==MF_UNCHECKED)
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d'or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDiscard,qsDone,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//5
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDone,qsSuspend,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//6
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_CHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_UNCHECKED//2
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDone,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//7
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDiscard,qsSuspend,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//8
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_UNCHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_CHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_UNCHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDiscard,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//9
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_UNCHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_UNCHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_CHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsSuspend,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//10
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_CHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_CHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_CHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDone,qsDiscard,qsSuspend,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//11
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_CHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_CHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_UNCHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDone,qsDiscard,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//12
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_UNCHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_CHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_CHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDiscard,qsSuspend,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//13
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_CHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_UNCHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_CHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s' and (R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d' or R.status='%d') and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,qsDone,qsSuspend,qsInLine,qsDoing,qsPrepare,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//14
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_CHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_CHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_CHECKED//3
	...
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.room_id,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,Q.his_queue_id,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,W.room_name waitroom_name,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id left join WaitingRoom W on Q.room_id=W.room_id) R where R.office_id='%s'and regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) %s order by R.status,R.priority desc,R.regtime")
			,officeid,theApp.GetController()->GetDataDays(), m_bReadInPatient ? _T("") : _T("and (in_flag<>1 or in_flag is null)"));
	}//15
	else if(MainFrame->m_menuCheckStates[ID_FR_SHOWCALL]==MF_UNCHECKED//1
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWQUIT]==MF_UNCHECKED//2
		&& MainFrame->m_menuCheckStates[ID_FR_SHOWSUSPEND]==MF_UNCHECKED//3
...
BOOL COfficeQueView::SearchData(CString &date,CString& contex,int index)
{
	...
	if(index==0)
	{
		sql.Format(_T("select * from (select Q.gender_flag,Q.office_id,Q.regtime,datediff(ss,Q.regtime,getdate()) wait_seconds,Q.status,Q.priority,Q.queue_id,Q.patient_name,D.name doctor_name,O.call_id,Q.log_id,Q.queue_id_call,Q.his_queue_id,Q.id_pay,Q.pay_desc,Q.in_flag from Queue Q left join Office O on Q.office_id=O.office_id left join Doctor D on Q.doctor_id=D.doctor_id) R where R.log_id='%s'and '%s'=convert(varchar(10),R.regtime,120) and R.office_id='%s' order by R.status,R.his_queue_id,R.priority")
