...
public:
	...
	void SetConnectionTimeout(long nConnectionTimeout = 3)
...
public:
	...
	BOOL SetFieldValueByColNum(int nIndex, CString strValue);
	...
	void MoveTo(LONG iItem)
		{m_pRecordset->Move(iItem, (LONG)adBookmarkFirst);};
...
/*
	CADOException(const CADOException& that):
		m_strError(that.m_strError),
		m_lErrorCode(that.m_lErrorCode)
		{
