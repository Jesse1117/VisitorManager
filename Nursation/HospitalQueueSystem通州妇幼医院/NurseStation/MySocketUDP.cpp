...
BOOL MySocketUDP::SendTo( CHAR * pData, UINT nLen, CString wszIP, USHORT uPort)
{
	...
	CommonStrMethod::WChar2Char(szIP, _MY_MAX_IP_LENGTH, wszIP);
...
CString MySocketUDP::NetToHostIP(IN_ADDR& in_addrNet)
{
	...
	CommonStrMethod::Char2WChar(wszIP, _MY_MAX_IP_LENGTH, pszIP);
	//CString strIP(wszIP);
	//return strIP;
...
void MySocketUDP::WriteLogWithTime(CString strSockLog)
{
	...
	{
		if(!CommonStrMethod::CreatePathMy(m_strLogFilePath))
		{
			return;
		}
	}
