...
void CPaientView::OnInitialUpdate()
{
	...
	rect.right-=2;
	int iCol = 0;
	m_view_paient.InsertColumn(iCol++,_T("序号"),LVCFMT_CENTER,rect.Width()*5/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("就诊号"),LVCFMT_CENTER,rect.Width()*14/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("排队号码"),LVCFMT_CENTER,rect.Width()*7/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("患者姓名"),LVCFMT_CENTER,rect.Width()*7/100,-1);
	//m_view_paient.InsertColumn(iCol++,_T("等候室"),LVCFMT_CENTER,rect.Width()*8/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("医生姓名"),LVCFMT_CENTER,rect.Width()*7/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("当前状态"),LVCFMT_CENTER,rect.Width()*7/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("优先级"),LVCFMT_CENTER,rect.Width()*6/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("检查项目"),LVCFMT_CENTER,rect.Width()*15/100,-1);
	//m_view_paient.InsertColumn(iCol++,_T("项目编号"),LVCFMT_CENTER,rect.Width()*7/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("科别"),LVCFMT_CENTER,rect.Width()*5/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("排队时间"),LVCFMT_CENTER,rect.Width()*15/100,-1);
	m_view_paient.InsertColumn(iCol++,_T("已等候"),LVCFMT_CENTER,rect.Width()*10/100,-1);
...
//排队操作
BOOL CPaientView::WriteDataToMyQueue(CString pait_logid)
{
	if(pait_logid==_T(""))
	{
		return FALSE;
	...
	try
	{
		...
		{
			CADORecordset *pre=new CADORecordset(&db);
			//排重查找在HisInterface中没有加入Queue科室排队的数据
			CString sql=_T("");

			sql.Format(_T("select * from (select distinct h.log_ID,h.deptName,h.Queue,h.docName,h.Times,d.doctor_id,o.name officename,o.call_id,o.office_id from HisInterface h left join Office o on o.office_id=h.Queue left join Doctor d on h.docName=d.name) h where regtime>cast(CONVERT(varchar(100),dateadd(day,-%d,GETDATE()),23) as datetime) and h.log_ID not in (select log_id from Queue where status<>'%d') and h.office_id in(select o.office_id from Nurse_Office n,Office o where Nurse_id='%s' and n.office_id=o.office_id) and h.log_ID='%s' order by h.Times"),
				theApp.GetController()->GetDataDays(), qsDone,theApp.GetLoginID(),pait_logid);
			...
			pre->GetFieldValue(_T("log_ID"),log_ID);
			pre->GetFieldValue(_T("deptName"),deptName);
			m_AddPaitName=deptName;
			pre->GetFieldValue(_T("Queue"),Queue);
			pre->GetFieldValue(_T("Queue"),m_AddPaitOfficeID);
			pre->GetFieldValue(_T("doctor_id"),docID);
			pre->GetFieldValue(_T("doctor_id"),m_AddPaitDocID);
			pre->GetFieldValue(_T("call_id"),call_id);
			pre->GetFieldValue(_T("docName"),m_AddPaitDocName);
			pre->GetFieldValue(_T("officename"),m_AddPaitOfficeName);
			...
			while(pos!=NULL)
			{
				...
				if(office.GetQueID()==Queue)
				{
					...
					if(NumStart<=NumEnd && queNumber<DayLimit && NumStart>=0 && 
						NumEnd>=0 && DayLimit>=0 && NumEnd>=NumStart)
					{
						...
						if(docID!=_T(""))
						{
							insertSql.Format(_T("insert into Queue(log_id,queue_id,patient_name,office_id,doctor_id,regtime,status,priority,queue_id_call) values('%s','%d','%s','%s','%s',getdate(),'%d','%d','%s')"),
								log_ID,NumStart,deptName,Queue,docID,qsInLine,priZero,call_id);
						}
						else
						{
							insertSql.Format(_T("insert into Queue(log_id,queue_id,patient_name,office_id,regtime,status,priority,queue_id_call) values('%s','%d','%s','%s',getdate(),'%d','%d','%s')"),
								log_ID,NumStart,deptName,Queue,qsInLine,priZero,call_id);
						}
						...
						office.SetQueNumber(queNumber);
						m_list_office.SetAt(posLast,office);
						break;
					}
				}
			}
	...
	{
		WriteLog::WriteDbErrLog(_T("CPaientView::WriteDataToMyQueue"));
	}
	return TRUE;
}

/*
void CPaientView::ReadData()
{
	CMainFrame* pMainFrame=((CNurseStationApp*)AfxGetApp())->m_pNurseWnd;
	CString sql=_T("");
	sql=_T("select Q.regtime,Q.status,Q.priority,Q.queue_id,P.name patient_name,D.name doctor_name from Queue Q,Patient P,Doctor D where Q.doctor_id=D.doctor_id and Q.patient_id=P.patient_id order by Q.status,Q.priority desc,Q.regtime");
	ShowData(sql);
}
*/
...
//					pRecordset->GetFieldValue(_T("waitroom_name"),waitroom_name);
					pRecordset->GetFieldValue(_T("id_pay"), strIdPay);
					pRecordset->GetFieldValue(_T("pay_desc"), strPayDesc);
					...
					int nWaitSeconds = CommonStrMethod::Str2Int(strWaitSeconds);
					...
					int iCol = 1;
					m_view_paient.SetItemText(item,iCol++,logid);
					m_view_paient.SetItemText(item,iCol++,que_id_call);
					m_view_paient.SetItemText(item,iCol++,patientname);
					//m_view_paient.SetItemText(item,iCol++,waitroom_name);
					m_view_paient.SetItemText(item,iCol++,doctorname);
					if(status==qsInLine)
					{
						m_view_paient.SetItemText(item,iCol,statusWaitLine);
						m_view_paient.SetItemColor(iCol,item,RGB(255,255,255));
					}
					else if(status==qsSuspend)
					{
						m_view_paient.SetItemText(item,iCol,statusSuspend);
						m_view_paient.SetItemColor(iCol,item,0x436EEE);
					}
					else if(status==qsDiscard)
					{
						m_view_paient.SetItemText(item,iCol,statusQuitQue);
						m_view_paient.SetItemColor(iCol,item,RGB(128,128,128));
					}
					else if(status==qsDone)
					{
						m_view_paient.SetItemText(item,iCol,statusDone);
						m_view_paient.SetItemColor(iCol,item,RGB(0,255,0));
					}
					else if(status==qsPrepare)
					{
						m_view_paient.SetItemText(item,iCol,statusPre);
						m_view_paient.SetItemColor(iCol,item,RGB(255,255,0));
					}
					else if(status==qsDoing)
					{
						m_view_paient.SetItemText(item,iCol,statusDoing);
						m_view_paient.SetItemColor(iCol,item,RGB(255,0,0));
					}
					iCol++;

					if(priority== priZero)
					{
						m_view_paient.SetItemText(item,iCol,priZeroText);
						m_view_paient.SetItemColor(iCol,item,RGB(255,255,255));
					}
					else if(priority== priOne)
					{
						m_view_paient.SetItemText(item,iCol,priOneText);
					}
					else if(priority== priTwo)
					{
						m_view_paient.SetItemText(item,iCol,priTwoText);
					}
					else if(priority== priThree)
					{
						m_view_paient.SetItemText(item,iCol,priThreeText);
						m_view_paient.SetItemColor(iCol,item,RGB(0,255,0));
					}
					iCol++;

					m_view_paient.SetItemText(item,iCol++,strPayDesc);
					//m_view_paient.SetItemText(item,iCol++,strIdPay);
					CString strInFlag;
					if(nInFlag == 1)
					{
						strInFlag = _T("住院");
					...
					m_view_paient.SetItemText(item,iCol++,strInFlag);
					m_view_paient.SetItemText(item,iCol++,regtime);
					m_view_paient.SetItemText(item,iCol++,CMinute);
...
void CPaientView::OnInsertqueue()
{
	...
	else
	{
		while(pos)
		{
			...
			{
				CString sql=_T("");
				sql.Format(_T("update queue set priority='%d' where log_id='%s'"),priThree,log_id);
				try
				{
					CADODatabase db;
					db.SetConnectionString(pMainFrame->m_strConn);
		...
		try
		{
			...
			while(pos)
			{
				...
				if(status==statusDone && !logid.IsEmpty())
				{
					...
					sql.Format(_T("select Q.doctor_id,D.name from Queue Q,Doctor D where log_id='%s' and Q.doctor_id=D.doctor_id"),logid);
...
BOOL CPaientView::ReviseListCtrDocName(CString paitLogid,CString docName)
{
	...
	for(int i=0;i<count;i++)
	{
		...
		if(paitLogid==logid)
		{
			m_view_paient.SetItemText(i,4,docName);
			break;
		}
	}
	return TRUE;
...
BOOL CPaientView::ReviseListCtrPatStatus(CString paitLogid,int status,CString DocName)
{
...
BOOL CPaientView::ReviseListCtrPatQueidcall(const CString& paitLogid,const CString& queidcall)
{
	...
	for(int i=0;i<count;i++)
	{
		...
		if(paitLogid==logid)
		{
			m_view_paient.SetItemText(i,2,queidcall);
			break;
		}
	}
	return TRUE;
...
BOOL CPaientView::ReviseListCtrPatName(const CString& paitLogid,const CString& newName)
{
	...
	for(int i=0;i<count;i++)
	{
		...
		if(paitLogid==logid)
		{
			m_view_paient.SetItemText(i,3,newName);
			break;
		}
	}
	return TRUE;
