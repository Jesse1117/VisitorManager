...
BEGIN_MESSAGE_MAP(CDoctorManageDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_REFRESH, &CDoctorManageDlg::OnBnClickedButtonRefresh)
...
void CDoctorManageDlg::OnBnClickedButtonSetPhoto()
{
	...
	if(IDOK == fdlg.DoModal())
	{
		...
		{
			m_set.Edit();
			m_set.SetFieldValue(_T("photo"), m_strPhotoPath);
		}
		catch (_com_error& e)
		{
...
void CDoctorManageDlg::OnBnClickedButtonAdd()
{
	...
	if(nStartNumIndex == -1)
	{
		...
		char szbuf[32] = {0};
		CommonStrMethod::WChar2Char(szbuf, 32, strMaxDocID.GetBuffer());
...
void CDoctorManageDlg::InitControls()
{
	...
	m_listDocList.InsertColumn(2, _T("姓名"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(3, _T("工号"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(4, _T("性别"), LVCFMT_LEFT, nColWidth * 5);
	m_listDocList.InsertColumn(5, _T("职称"), LVCFMT_LEFT, nColWidth * 9);
	m_listDocList.InsertColumn(6, _T("专家"), LVCFMT_LEFT, nColWidth * 5);
	m_listDocList.InsertColumn(7, _T("诊室"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(8, _T("照片"), LVCFMT_LEFT, nColWidth * 6);
	m_listDocList.InsertColumn(9, _T("呼叫声音"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(10, _T("准备声音"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(11, _T("呼叫显示"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(12, _T("准备显示"), LVCFMT_LEFT, nColWidth * 8);
	m_listDocList.InsertColumn(13, _T("特呼"), LVCFMT_LEFT, nColWidth * 5);
	m_listDocList.InsertColumn(14, _T("重呼"), LVCFMT_LEFT, nColWidth * 5);
	m_listDocList.InsertColumn(15, _T("呼叫器"), LVCFMT_LEFT, nColWidth * 7);
	m_listDocList.InsertColumn(16, _T("评价器"), LVCFMT_LEFT, nColWidth * 7);
...
void CDoctorManageDlg::RefreshList()
{
	try
	{
		...
		for(LONG iItem = 0;!m_set.IsEOF(); iItem++,m_set.MoveNext())
		{
			...
			int nValue;
			m_set.GetFieldValue(_T("gender"), nValue);
			m_listDocList.SetItemText(iItem, 4, GetGenderName(nValue));
			m_set.GetFieldValue(_T("title"), strValue);
			m_listDocList.SetItemText(iItem, 5, strValue);
			m_set.GetFieldValue(_T("expert_flag"), nValue);
			if(nValue == expertYes)
			{
				strValue = _T("是");
			...
			m_set.GetFieldValue(_T("consult_id"), strValue);
			m_listDocList.SetItemText(iItem, 7, GetConsultName(strValue));
			m_set.GetFieldValue(_T("photo"), strValue);
			m_listDocList.SetItemText(iItem, 8, GetTransStr(strValue));
			m_set.GetFieldValue(_T("sound_call"), strValue);
			m_listDocList.SetItemText(iItem, 9, GetTransStr(strValue));
			m_set.GetFieldValue(_T("sound_wait"), strValue);
			m_listDocList.SetItemText(iItem, 10, GetTransStr(strValue));
			m_set.GetFieldValue(_T("display_call"), strValue);
			m_listDocList.SetItemText(iItem, 11, GetTransStr(strValue));
			m_set.GetFieldValue(_T("display_wait"), strValue);
			m_listDocList.SetItemText(iItem, 12, GetTransStr(strValue));
			...
			m_listDocList.SetItemText(iItem, 14, CommonStrMethod::Int2Str(iValue));
			...
			m_listDocList.SetItemText(iItem, 16, strValue);
		}
...
int CDoctorManageDlg::ReadFileToDB(CString strFilePath)
{
	...
	for(;!file.eof()/*&&file.good()*/;)
	{
		memset(buf, 0, 2048);
		file.getline(buf, 2048);
		CommonStrMethod::Char2WChar(wbuf, 4096, buf);
...
void CDoctorManageDlg::OnBnClickedButtonRefresh()
{
	LoadOfficeList();
	LoadConsults();
	UpdateConsultList();
